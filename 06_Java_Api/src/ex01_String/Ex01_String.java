...
public class Ex01_String {
	...
	*/
	...
	public static void lastIndexOf() {
		...
		int idx2 = slogan.lastIndexOf("정병인");
	...
	public static void startsWith() {
		...
		// startWith
		// 문자열이 지정된 정규식 패턴(Reqular Expression)으로 시작하면 true 반환
		
		// endWith
		// 문자열이 지정된 정규식 패턴(Reqular Expression)으로 끝나면 true 반환
		
		// matches
		// 문자열이 지정된 정규식 패턴(Reqular Expression)을 포함하면 true 반환
		

		String name = "정병인";
		
		if(name.startsWith("정")) {
			System.out.println("정씨다.");
		}else {
			System.out.println("정씨가 아니다.");
		}
	...
	public static void isEmpty() {
		...
		// JDK 11 이후에서만 사용 가능
		// 23년 1월 현재 기준으로는 str.trim().isEmpty() 를 통해 작업을 해야한다.
		if(str.isBlank()) {
			System.out.println("빈 문자열이다.");
		}else {
			System.out.println("빈 문자열이 아니다");
	...
	public static void ex01() {
		...
		String requestURL = url.substring(0, url.indexOf("?")); // https://comic.naver.com/webtoon/detail 여기까지 출력
		System.out.println(requestURL);
		
		String params = url.substring(url.indexOf("?") + 1); // titleId=758037&no=112&weekday=mon 여기까지 출력
